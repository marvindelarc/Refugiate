...
<table cellpadding="0" cellspacing="0" border="0" class="table table-striped" id="datatable" width="100%">
                ...
                <tbody>
                 <%
                   List<clsIntalacion> lista=clsGestor.ListarIntalacion(true);
                   if(lista!=null)
                   for(clsIntalacion entidad : lista)
                   {
                 %>
                  <tr >
                    <td><%=entidad.getIdInstalacion()%></td>
                    <td><%=entidad.getObjServicio().getIdServicio()%></td>
                    ...
                    <td class="center">
                         <center> <a href="javascript:void(0)" onclick="edit_form(<%=entidad.getIdInstalacion()%>,<%=entidad.getObjSucursal().getIdSucursal()%>,<%=entidad.getObjServicio().getIdServicio()%>,'<%=entidad.getDescripcion()%>',<%=entidad.getEstado()%>)" class="btn btn-primary btn-sm btn-small">Editar</a></center> 
                    </td>
                  </tr>
                <%}%> 
                </tbody>
